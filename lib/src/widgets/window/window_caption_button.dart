...
class FixAutoWindowCaptionButton extends StatefulWidget {
  FixAutoWindowCaptionButton({
    super.key,
    this.brightness,
    this.icon,
    this.iconName,
  ...
  FixAutoWindowCaptionButton.minimize({
    super.key,
    this.brightness,
    this.icon,
    this.onPressed,
    this.type = WindowButtonType.minimize,
  }) : iconName = _kIconChromeMinimize;

  FixAutoWindowCaptionButton.maximize({
    super.key,
    this.brightness,
    this.icon,
    this.onPressed,
    this.type = WindowButtonType.maximize,
  }) : iconName = _kIconChromeMaximize;

  FixAutoWindowCaptionButton.unmaximize({
    super.key,
    this.brightness,
    this.icon,
    this.onPressed,
  ...
  FixAutoWindowCaptionButton.close({
    super.key,
    this.brightness,
    this.icon,
    this.onPressed,
    this.type = WindowButtonType.close,
  ...
  @override
  State<FixAutoWindowCaptionButton> createState() =>
      _FixAutoWindowCaptionButtonState();
}

class _FixAutoWindowCaptionButtonState extends State<FixAutoWindowCaptionButton>
    with WindowsInterfaceListener {
  ...
  void initState() {
    super.initState();
    windowsInterface.addListener(this);
  }

  @override
  void dispose() {
    super.dispose();
    windowsInterface.removeListener(this);
  }

  @override
  void onTitleButtonHoverd(String type) {
    if (widget.type.name == type) {
  ...
  Widget build(BuildContext context) {
    ...
    return MouseRegion(
      onExit: (value) => _onExit,
      onHover: (value) => _onHover,
...
class FixWindowCaptionButton extends StatefulWidget {
  final WindowButtonType type;
  final Brightness? brightness;

  const FixWindowCaptionButton({
    super.key,
    required this.type,
    this.brightness,
  });
  ...
  @override
  State<FixWindowCaptionButton> createState() => _FixWindowCaptionButtonState();
}

class _FixWindowCaptionButtonState extends State<FixWindowCaptionButton> {
  Widget buildMaximize(BuildContext context) {
  ...
  Widget buildMinimize(BuildContext context) {
    return FixAutoWindowCaptionButton.minimize(
      brightness: widget.brightness,
      onPressed: () async {
  ...
  Widget buildClose(BuildContext context) {
    return FixAutoWindowCaptionButton.close(
      brightness: widget.brightness,
      onPressed: () {
        windowManager.close();
      },
  ...
  Widget build(BuildContext context) {
    ...
    switch (widget.type) {
      case WindowButtonType.minimize:
        content = buildMinimize(context);
        break;
      case WindowButtonType.maximize:
        content = buildMaximize(context);
        break;
      case WindowButtonType.close:
        content = buildClose(context);
        break;
      default:
